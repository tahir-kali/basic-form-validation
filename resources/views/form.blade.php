...
<body>
<form action="store" class="form" method="post" name="fields" enctype='multipart/form-data'>
    <h1>Global Form</h1>
    @if(isset($success))
        <div>
            {{ $success  }}
                    ...
                    <div class="input">
                        ...
                        @if($field['slug'] === 'images')
                          <div>
                              @foreach(range($field['element']['params']['min'], $field['element']['params']['max']) as $number)
                                  <input type="file" name="fields[{{ $field['id'] }}][{{ $number }}]" style="width: 100%;" />
                                  <div class="error">
                        ...
                        @if($field['slug'] === 'radio')
                            @if(isset($field['values']))
                                @foreach($field['values'] as $key=>$val)
                                    <div>
                                        <input type="radio" name="fields[{{$field['id']}}]" value="{{$val['value']}}"/>
                                        <span>{{$val['label']}}</span>
                                        <div class="error">
                                            @error("fields.".$field['id'].".".$key)
                                            <div class="alert alert-danger">{{ $message }}</div>
